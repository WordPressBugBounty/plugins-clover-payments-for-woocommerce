...
class WOO_CLV_ADMIN_CAPTURE {
	...
	public static function bulk_actions_capture_option( $actions ) {
		$actions['wc_capture_charge'] = __( 'Capture Charge', 'clover-payments-for-woocommerce' );
		return $actions;
	}
	...
	public static function notice_bulk_actions_capture() {
		...
		if (
			...
			if ( $count ) {
				?>
						<div id="message" class="updated fade"><p><?php echo esc_html_e( 'Orders captured via Clover.', 'clover-payments-for-woocommerce' ) . '(' . esc_attr( $count ) . ').'; ?></p></div>
				<?php
			...
			if ( $fcount ) {
				?>
						<div id="message" class="updated fade clv-error" style="border-left-color: #a00;"><p><?php echo esc_html_e( 'Transaction could not be processed through Clover.', 'clover-payments-for-woocommerce' ) . '(' . esc_attr( $fcount ) . ').'; ?></p></div>
				<?php
